...
Логично, если бы тот, кто отправляет сообщение и создавал бы очередь. Однако, если при развертывании у нас sneakers-обработчики не обнаружат очереди - они упадут с ошибкой. Поэтому в нашем приложении давайте, у нас будет создавать очередь то приложение, которое прослушивает очередь.
