...
Микросервис для бюджетного контроля
...
COMPOSE_DOCKER_CLI_BUILD=1 DOCKER_BUILDKIT=1 docker compose run -e "RAILS_ENV=development" budget-control bundle exec rake db:drop db:create db:migrate
```
...
```bash
docker compose exec -it budget-control bash
```
