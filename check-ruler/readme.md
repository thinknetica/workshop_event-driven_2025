...
Сервис извлекающий цену из распознанного текста и принимающий решение, отправлять далее покупку на регистрацию или на бюджетный контроль.
...
COMPOSE_DOCKER_CLI_BUILD=1 DOCKER_BUILDKIT=1 docker compose run -e "RAILS_ENV=development" check-ruler bundle exec rake db:drop db:create db:migrate
```
...
```bash
docker compose exec -it check-ruler bash
```
